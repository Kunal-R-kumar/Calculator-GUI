...
def piccorector(abc):
    return ImageTk.PhotoImage(Image.open(f'{os.path.dirname(os.path.abspath(__file__))}\\{abc}.png').resize((65,50)))
def binder(value):
...
def bkt():
    ...
    e.icursor(idx+1)
    e.focus_set()
#tkinter basic window & required images with entry and button widgets
base=Tk()
...
kl=Label(text="You can use Keyboard too",font=("Times 13"),fg="#9CC9FC",bg="black");
kl.pack(anchor=W)
kl.after(10000,lambda:[kl.pack_forget()])
